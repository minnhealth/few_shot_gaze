...
class frame_processer:
        ...
        while ret:
            ...
            if len(face_location) > 0:
                ...
                if show:
                    ...
                    #cv2.moveWindow("por", 3840, 0) # Uncomment to run on a second/third monitor
    ...
    def process_one(self, subject, img, mon, device, gaze_network, por_available=True, output_pixels=True):
        g_t = None
        data = {'image_a': [], 'gaze_a': [], 'head_a': [], 'R_gaze_a': [], 'R_head_a': []}
        if output_pixels and por_available:
            data['pixel_gt'] = []
            data['pixel_pred'] = []
        if por_available:
            f = open('./%s_calib_target.pkl' % subject, 'rb')
        ...
        if len(face_location) > 0:
            ...
            # compute pixel coords if requested
            if output_pixels and por_available:
                # compute eye gaze and point of regard
                for k, v in input_dict.items():
                    input_dict[k] = torch.FloatTensor(v).to(device).detach()
                ...
                # predicted monitor coords
                data['pixel_pred'].append((x_pixel_hat, y_pixel_hat))
        return data
